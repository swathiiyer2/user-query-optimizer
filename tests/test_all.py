    ...
    correct_ops = {
        ...
        'test-query-2.txt': {1: ['select columns explicitly'],
                             3: ["Filtering on ordered columns can improve performance. Here are some that you aren't filtering on: submission_date_s3",
                                 'Transformations on partitioned columns are more computationally expensive than direct comparisons. Try and move the transformation to the other side of the comparison on the following column: submission_date_s3']},
